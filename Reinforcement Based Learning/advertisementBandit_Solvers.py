...
class Solver(object):
    def __init__(self, bandit):
        ...
        """
        assert isinstance(bandit, AdvertisementBandit)
...
class EpsilonGreedy(Solver):
    ...
    def run_one_step(self):
        ...
        # connect to enviorment
        r = self.bandit.generate_reward(i)

        # updae estimate of mean
...
class UCB1(Solver):
    ...
    def run_one_step(self):
        ...
        # find the index of the option that has the maximum upper confidence bound value
...
class BayesianUCB(Solver):
    ...
    def run_one_step(self):
        ...
        max_reward = -np.inf
        for k in range(self.bandit.n):
            # CurrEstimate = self._as[k] / float(self._as[k] + self._bs[k]) + beta.std(self._as[k], self._bs[k]) * self.c
